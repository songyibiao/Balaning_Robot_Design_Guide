# 提高PWM频率为24kHz，控制范围为0~999

经过上面的一系列实验下来，我们已经初步调好了两轮自平衡小车的平衡功能。但是，不知道你们有没有发现几个问题：
...
> 3. 小车加入速度环后，已经具备了一定抗干扰能力，但是可能会缓慢往一边持续运动，无法做到原地平衡。

### 提高 PWM 频率为 24kHz

既然 10kHz 是人耳能听到的工作频率，那么我们可以尝试提高 PWM 频率来解决这个问题。

进入我们上一小节实验的 MiaowLabs-Demo 文件夹，找到 MiaowLabs-Demo.ioc 工程文件，双击，打开工程。在 Pinout&Configuration 界面中的 Timers 里点击 TIM3，在 Parameter Settings 选项卡里将 Prescaler 改成 2，Counter Period 改成 999。此时，PWM = 72000000 / (2+1) / (999+1) = 24000 = 24kHz。 

![修改PWM频率](/img/2021-06-12_100047.png)
...
这样一来，我们 PWM 的可控范围不再是 0~99，而是变成了 0~999。也就是说，当设为 PWM 设为 999 时，占空比才是 100%。
...
到这里，速度环也调好了，但是有些小车由于安装误差和传感器误差等因素，小车会以低速一直往一个方向走。这个问题是由于我们在角度环中没有修正偏移值导致的。因为没有调到这一步，很难看出机械零点的作用和意义，我们在这里适当加上机械零点偏移值。

我们打开 control.c 文件，加上机械零点偏移值的宏定义。

```c
#define CAR_ZERO_ANGLE  (g_fCarAngleOffset)  //机械零点偏移值
float g_fCarAngleOffset=5.0;//每辆小车的机械零点都不一定相同
```

我们先用一个宏定义 CAR_ZERO_ANGLE 封装变量 g_fCarAngleOffset，然后 g_fCarAngleOffset 在这里赋值为 5。

可以看到上面的定义中，机械零点偏移值被赋值为 5 ，通常在我们小车的机械零点的范围为 -5~5，不会太大。过小过大都有可能导致小车最终缓慢往一边运动，无法在原地平衡。每台小车的机械零点都不完全一样，我们可以从 0 开始往两边尝试，看看哪个机械零点偏移值更适合。如果小车是往前走（超声波模块方向为前，OLED方向为后），则可以赋值为正，比如1；如果小车往后走，可以赋值为负，比如-1。

找到角度计算函数 AngleCalculate(void) ，在里面减去机械零点偏移值。
...
至此，我们加上机械零点偏移值，可以看到小车不会再缓慢往一边一直运动，达到了我们的期望中的样子。
